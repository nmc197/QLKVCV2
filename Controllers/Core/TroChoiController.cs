...
namespace QuanLyKVC.Controllers
{
    [Route("[controller]")]
    [ApiController]
    public class TroChoiController : ControllerBase
    {
        ITroChoiRepository repository;
        public TroChoiController(ITroChoiRepository _repository)
        {
        ...
        [HttpPost]
        [Route("api/Add")]
        public async Task<IActionResult> Add([FromBody]Trochoi model)
        {
        ...
        [HttpPost]
        [Route("api/Update")]
        public async Task<IActionResult> Update([FromBody]Trochoi model)
        {
            if (ModelState.IsValid)
            {
                try
        ...
        [HttpPost]
        [Route("api/Delete")]
        public async Task<IActionResult> Delete([FromBody]Trochoi model)
        {
            if (ModelState.IsValid)
            {
                try
        ...
        public async Task<IActionResult> DeletePermanently([FromBody]Trochoi model)
        {
            int result = 0;

            if (!(model.MaTroChoi != null))
            {
                return BadRequest();
            }
            ...
            try
            {
                ...
                result = await repository.DeletePermanently(model.MaTroChoi);
                if (result == 0)
                {
                    return NotFound();
                }
        ...
        {
            int result = repository.CountTroChoi();
            return result;
        }
    }
}
