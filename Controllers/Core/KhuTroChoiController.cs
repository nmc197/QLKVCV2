...
namespace QuanLyKVC.Controllers
{
    ...
    public class KhuTroChoiController : ControllerBase
    {
        ...
        public async Task<IActionResult> DeletePermanently([FromBody]Khutrochoi model)
        {
            int result = 0;
            ...
            try
            {
                ...
                result = await repository.DeletePermanently(model.MaKhu);
                if (result == 0)
                {
                    return NotFound();
                }
        ...
        {
            int result = repository.CountKhuTroChoi();
            return result;
        }
    }
}
